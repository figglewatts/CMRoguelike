...
import uk.uncodedstudios.CMJamGame.Character.Player;
import uk.uncodedstudios.CMJamGame.Enemy.BaseEnemy;
import uk.uncodedstudios.CMJamGame.Enemy.EnemyTextures;
	...
	public static void Generate() {
		...
		for (int r = 0; r < MAX_ROOMS; r++)
		{
			...
			if (failed == false) {
				...
				if (numberOfRooms == 0) {
					CMJamGame.player.setxPos(newX*Tile.RenderTileWidth);
					CMJamGame.player.setyPos(newY*Tile.RenderTileHeight);
					setCameraToPlayer(newX, newY);
					MessageBox.message(Camera.Location + ": top lel?");
					MessageBox.message(CMJamGame.player.getxPos() + " " + CMJamGame.player.getyPos());
				} else {
		...
		CMJamGame.numberOfMonsters = numberOfMonsters;
	}
